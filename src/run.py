...
def print_image_matching_stats(dataset_folder: str, ground_truth_path: str):
    ...
    image_files_rel = [str(p.relative_to(dataset_root)) for p in image_files]
    print(f"[INFO] Tổng số file ảnh tìm thấy: {len(image_files_rel)}")
    # Đọc image_id từ CSV
    gt_csv_path = Path(ground_truth_path).expanduser().resolve()
    image_ids = set()
    with open(gt_csv_path, "r", newline="", encoding="utf-8") as f:
    ...
    # So khớp
    matched = set(image_files_rel) & image_ids
    print(
        f"[INFO] Số lượng image_id khớp với file ảnh: {len(matched)} / {len(image_ids)} ({len(matched) / max(1, len(image_ids)) * 100:.2f}%)"
    )
    print(
        f"[INFO] Số lượng file ảnh khớp với image_id: {len(matched)} / {len(image_files_rel)} ({len(matched) / max(1, len(image_files_rel)) * 100:.2f}%)"
    )
